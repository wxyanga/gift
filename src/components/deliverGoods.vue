<template>
	<div>
		<el-form :inline="true" label-width="100px">
			...
			<el-form-item label="使用快递">
				<el-select v-model="form.expressType">
					...
					<el-option v-for="(item, index) in useExpress" :key="index" :label="item.codeName" :value="item.codeValue"></el-option>
				</el-select>
			</el-form-item>
		...
		<el-table :data="tableData" @selection-change="handleSelectionChange">
			...
			<el-table-column prop="org_name" label="发货站"></el-table-column>
...
export default {
	...
	methods: {
		...
		dialogResendConfirm(row) {
			...
			this.$post(this.$API.URL_PUT_ORDER_CHECK_RESEND, params, '').then(res => {
				if (res.resend - 0 > 0) this.$message.success('重新发货成功')
				else this.$message.error('重新发货失败')
				this.id = []
				this.getTableData()
			})
		},
