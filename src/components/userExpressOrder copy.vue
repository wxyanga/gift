<template>
	<div>
		<div style="text-align: center;background:#ccc;padding:20px;margin-bottom:20px">
			...
			<el-tab-pane label="Excel录入" name="first">
				<el-form label-width="150px">
					<el-form-item label="设置发货信息：">
						<el-autocomplete style="width:80%" v-model="validatorForm.setNewAddress" :fetch-suggestions="querySearch" :debounce="100" @select="handleSelect" @clear="clearText">
				...
				<el-form :model="validatorForm" ref="form2" class="cust-exp-type" :rules="rules2" label-width="150px">
						...
						<el-select v-model="validatorForm.ExpTypeLabel" style="width:100%" @change="changeSeparateLabel">
							<el-option v-for="(it, ind) in separate" :key="ind" :value="it.text">
								<div style="position: relative;">
									...
									<el-button v-else type="text" class="set-default" @click="setDefaultExp(it)">设为默认</el-button>
								</div>
							</el-option>
						</el-select>
					</el-form-item>
					<el-form-item label="Excel录入单据信息" style="width:80%">
						...
						<el-button type="primary" style="float: right;" :disabled="!setDefaultExpType.id || ExpTable.length == 0" @click="upEXP"
							>共 {{ ExpTable.length }} 个信息，下单需使用 {{ (setDefaultExpType.myMoney || 0) * (ExpTable.length || 0) }} 元，账户余额：{{ userInfo.bal }}，确认金额并下单</el-button
						>
					</el-form-item>
				</el-form>
				<el-table :data="ExpTable">
					<el-table-column prop="goodsName" label="商品名称"></el-table-column>
					<el-table-column prop="tid" label="电商订单号"></el-table-column>
					...
					<el-table-column prop="receiver" label="收货人姓名"></el-table-column>
					<el-table-column prop="receiverTel" label="收货人电话"></el-table-column>
					...
					<el-table-column prop="receiveAddr" label="收货人地址"></el-table-column>
					<el-table-column prop="receiverPostid" label="收货邮编"></el-table-column>
			...
			<el-tab-pane label="批量录入" name="second">
				...
				<el-form :model="validatorForm" ref="form2" class="cust-exp-type" :rules="rules2" label-width="150px">
					...
					<el-form-item label="录入参照格式">
						<div>
							<span style="background:green;color:#fff;padding:5px 10px">电商平台订单号 商品名称 收货人姓名 收货人电话 收货详细地址 收货邮编。</span>
							<br />
							<span style="color:red">末尾使用中文句号,中间使用空格,位置必须严格一致</span>
						</div>
					</el-form-item>
					<el-form-item label="手动录入单据信息" style="width:80%">
						<el-input type="textarea" v-model="textAreaValue" placeholder="请参照录入格式输入" :rows="10"></el-input>
					</el-form-item>
...
export default {
	...
	methods: {
		...
		analysis() {
			let alalysisArr = this.textAreaValue.replace(/\s+/g, ' ').split('。')
			alalysisArr.forEach(item => {
				let arr = item.split(' ').filter(it => it)
		...
		getDefaultData() {
			this.$post(this.$API.URL_GET_USER_ADDRESS, {}, '').then(res => {
				this.sendAddr = res
				let f = this.sendAddr.filter(item => item.id == this.userInfo.defaultAddr.id)
				// let f = []
		...
		upEXP() {
			this.$post(this.$API.URL_GET_USER_INFO, {}, '').then(res => {
				...
				if (sel && sel.prov && sel.county && sel.address) {
					...
					if (res.bal - allBal >= 0) {
						let length = this.ExpTable.length
						let postLength = 0
						for (const iterator of this.ExpTable) {
							let params = {
								...iterator,
								expressType,
								amt,
		...
		setDefault() {
			...
			this.$log(0, this.validatorForm.setSelectObj.id, this.userInfo.defaultAddr.id, this.senderForm, this.userInfo.defaultAddr)
